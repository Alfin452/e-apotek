<x-superadmin-layout>
    <div class="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <h1 class="text-3xl font-extrabold text-slate-900 tracking-tight">Tren Volume Penjualan</h1>
            <p class="text-slate-500 mt-1 font-medium">Analisis jumlah transaksi dan pendapatan selama 30 hari terakhir.</p>
        </div>
        <div>
            <a href="{{ route('superadmin.reports.sales_trend', ['export' => 'pdf']) }}" target="_blank" class="inline-flex items-center gap-2 bg-brand-blue hover:bg-slate-800 text-white font-bold py-2.5 px-6 rounded-xl transition-colors shadow-sm text-sm">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm text-left">
                <thead class="text-xs text-slate-500 uppercase bg-slate-50 border-b border-slate-100 font-bold">
                    <tr>
                        <th class="px-6 py-4 text-center">Tanggal</th><th class="px-6 py-4 text-center">Total Transaksi</th><th class="px-6 py-4 text-center">Total Pendapatan (Rp)</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-100">
                    @forelse($tableData as $row)
                    <tr class="hover:bg-slate-50/50 transition-colors">
                        <td class="px-6 py-4 font-bold text-slate-900">{{ $row['date'] }}</td><td class="px-6 py-4 text-center">{{ $row['trx'] }}</td><td class="px-6 py-4 text-right">{{ number_format($row['revenue'], 0, ',', '.') }}</td>
                    </tr>
                    @empty
                    <tr>
