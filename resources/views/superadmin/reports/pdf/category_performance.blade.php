...
<head>
    <meta charset="utf-8">
    <title>Kinerja Kategori Obat</title>
    <style>
...
<body>
    <div class="header">
        <h1 class="title">LAPORAN: KINERJA KATEGORI OBAT</h1>
    ...
    <table>
        <thead>
            <tr>
                <th>Kategori</th><th>Total Qty Terjual</th><th>Total Pendapatan (Rp)</th>
            </tr>
        </thead>
        <tbody>
            @forelse($tableData as $row)
            <tr>
                <td class="text-left">{{ $row->name }}</td><td class="text-center">{{ $row->total_qty }}</td><td>{{ number_format($row->total_revenue, 0, ',', '.') }}</td>
            </tr>
            @empty
            <tr>
