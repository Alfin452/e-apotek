...
<head>
    <meta charset="utf-8">
    <title>Kinerja Kasir & Pegawai</title>
    <style>
...
<body>
    <div class="header">
        <h1 class="title">LAPORAN: KINERJA KASIR & PEGAWAI</h1>
    ...
    <table>
        <thead>
            <tr>
                <th>Nama Pegawai</th><th>Total Transaksi</th><th>Total Pendapatan (Rp)</th>
            </tr>
        </thead>
        <tbody>
            @forelse($tableData as $row)
            <tr>
                <td class="text-left">{{ $row->name }}</td><td class="text-center">{{ $row->total_trx }}</td><td>{{ number_format($row->total_revenue, 0, ',', '.') }}</td>
            </tr>
            @empty
            <tr>
