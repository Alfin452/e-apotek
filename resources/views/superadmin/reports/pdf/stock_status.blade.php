...
<head>
    <meta charset="utf-8">
    <title>Status Stok Kritis</title>
    <style>
...
<body>
    <div class="header">
        <h1 class="title">LAPORAN: STATUS STOK KRITIS</h1>
    ...
    <table>
        <thead>
            <tr>
                <th>Nama Obat</th><th>Stok Saat Ini</th><th>Batas Minimum</th>
            </tr>
        </thead>
        <tbody>
            @forelse($criticalStocks as $row)
            <tr>
                <td class="text-left">{{ $row->name }}</td><td class="text-center">{{ $row->stock }}</td><td class="text-center">{{ $row->min_stock }}</td>
            </tr>
            @empty
            <tr>
