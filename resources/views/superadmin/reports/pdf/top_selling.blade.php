...
<head>
    <meta charset="utf-8">
    <title>Obat Terlaris (Kuantitas)</title>
    <style>
...
<body>
    <div class="header">
        <h1 class="title">LAPORAN: OBAT TERLARIS (KUANTITAS)</h1>
    ...
    <table>
        <thead>
            <tr>
                <th>Nama Obat</th><th>Total Qty Terjual</th><th>Total Pendapatan (Rp)</th>
            </tr>
        </thead>
        <tbody>
            @forelse($tableData as $row)
            <tr>
                <td class="text-left">{{ $row->name }}</td><td class="text-center font-bold">{{ $row->total_qty }}</td><td>{{ number_format($row->total_revenue, 0, ',', '.') }}</td>
            </tr>
            @empty
            <tr>
