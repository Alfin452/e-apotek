...
<head>
    <meta charset="utf-8">
    <title>Analisis Belanja Pemasok</title>
    <style>
...
<body>
    <div class="header">
        <h1 class="title">LAPORAN: ANALISIS BELANJA PEMASOK</h1>
    ...
    <table>
        <thead>
            <tr>
                <th>Nama Pemasok</th><th>Jumlah Transaksi</th><th>Total Pengeluaran (Rp)</th>
            </tr>
        </thead>
        <tbody>
            @forelse($tableData as $row)
            <tr>
                <td class="text-left">{{ $row->name }}</td><td class="text-center">{{ $row->total_trx }}</td><td>{{ number_format($row->total_spending, 0, ',', '.') }}</td>
            </tr>
            @empty
            <tr>
