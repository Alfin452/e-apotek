...
<head>
    <meta charset="utf-8">
    <title>Tren Volume Penjualan</title>
    <style>
...
<body>
    <div class="header">
        <h1 class="title">LAPORAN: TREN VOLUME PENJUALAN</h1>
    ...
    <table>
        <thead>
            <tr>
                <th>Tanggal</th><th>Total Transaksi</th><th>Total Pendapatan (Rp)</th>
            </tr>
        </thead>
        <tbody>
            @forelse($tableData as $row)
            <tr>
                <td class="text-left">{{ $row['date'] }}</td><td class="text-center">{{ $row['trx'] }}</td><td>{{ number_format($row['revenue'], 0, ',', '.') }}</td>
            </tr>
            @empty
            <tr>
