<x-superadmin-layout>
    <div class="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <h1 class="text-3xl font-extrabold text-slate-900 tracking-tight">Kinerja Kategori Obat</h1>
            <p class="text-slate-500 mt-1 font-medium">Analisis pendapatan berdasarkan kategori obat.</p>
        </div>
        <div>
            <a href="{{ route('superadmin.reports.category_performance', ['export' => 'pdf']) }}" target="_blank" class="inline-flex items-center gap-2 bg-brand-blue hover:bg-slate-800 text-white font-bold py-2.5 px-6 rounded-xl transition-colors shadow-sm text-sm">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm text-left">
                <thead class="text-xs text-slate-500 uppercase bg-slate-50 border-b border-slate-100 font-bold">
                    <tr>
                        <th class="px-6 py-4 text-center">Kategori</th><th class="px-6 py-4 text-center">Total Qty Terjual</th><th class="px-6 py-4 text-center">Total Pendapatan (Rp)</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-100">
                    @forelse($tableData as $row)
                    <tr class="hover:bg-slate-50/50 transition-colors">
                        <td class="px-6 py-4 font-bold text-slate-900">{{ $row->name }}</td><td class="px-6 py-4 text-center">{{ $row->total_qty }}</td><td class="px-6 py-4 text-right">{{ number_format($row->total_revenue, 0, ',', '.') }}</td>
                    </tr>
                    @empty
                    <tr>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            let bgColors = '#10b981';
            if ('doughnut' === 'pie' || 'doughnut' === 'doughnut') {
                bgColors = [
