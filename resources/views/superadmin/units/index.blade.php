<x-superadmin-layout>
    <div class="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <h1 class="text-3xl font-extrabold text-slate-900 tracking-tight">Unit & Satuan Obat</h1>
            <p class="text-slate-500 mt-1">Kelola daftar satuan fisik obat-obatan (misal: Strip, Botol, Tablet).</p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="bg-white rounded-3xl shadow-sm border border-slate-100 p-6 sticky top-8">
                ...
                <form action="{{ route('superadmin.units.store') }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="name" required placeholder="Misal: Strip, Botol, Pcs" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white focus:ring-2 focus:ring-brand-blue/20 focus:border-brand-blue transition-all text-sm font-medium text-slate-800">
                    </div>
        ...
        <!-- Table Data -->
        <div class="lg:col-span-2" x-data="{ editModalOpen: false, editId: '', editName: '', editUrl: '' }">
            <div class="bg-white rounded-3xl shadow-sm border border-slate-100 p-6 overflow-hidden">
                <div class="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4 border-b border-slate-100 pb-4">
                    <h3 class="text-lg font-bold text-slate-800">Daftar Unit / Satuan</h3>
                    <form action="{{ route('superadmin.units.index') }}" method="GET" class="w-full sm:w-64 relative">
                        <input type="text" name="search" value="{{ $search }}" placeholder="Cari unit..." class="w-full pl-10 pr-4 py-2 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white focus:ring-2 focus:ring-brand-blue/20 focus:border-brand-blue transition-all text-sm font-medium">
                    ...
                    <table class="w-full text-left text-sm text-slate-600">
                        ...
                        <tbody class="divide-y divide-slate-100">
                            ...
                            <tr class="hover:bg-slate-50 transition-colors">
                                <td class="px-4 py-3 font-medium text-slate-400">{{ $units->firstItem() + $index }}</td>
                                <td class="px-4 py-3 font-bold text-slate-800">{{ $unit->name }}</td>
                                <td class="px-4 py-3 text-center">
                                    <span class="inline-flex items-center justify-center px-2.5 py-1 rounded-full text-xs font-bold {{ $unit->medicines_count > 0 ? 'bg-blue-50 text-brand-blue border border-blue-100' : 'bg-slate-100 text-slate-500' }}">
                                        {{ $unit->medicines_count }} Obat
                                    </span>
                                </td>
                                <td class="px-4 py-3 text-right">
                                    <div class="flex items-center justify-end gap-2">
                                        <!-- Edit Button (triggers Alpine Modal) -->
                                        <button type="button" @click="editModalOpen = true; editId = '{{ $unit->id }}'; editName = '{{ addslashes($unit->name) }}'; editUrl = '/superadmin/units/{{ $unit->id }}'" class="p-2 text-slate-400 hover:text-brand-blue hover:bg-blue-50 rounded-lg transition-colors">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
                                        </button>
                                        
                                        <!-- Delete Form -->
                                        <form action="{{ route('superadmin.units.destroy', $unit->id) }}" method="POST" class="inline-block confirm-form" data-action="menghapus unit {{ $unit->name }}">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="p-2 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors {{ $unit->medicines_count > 0 ? 'opacity-50 cursor-not-allowed' : '' }}" {{ $unit->medicines_count > 0 ? 'disabled title=Data_Masih_Digunakan' : '' }}>
