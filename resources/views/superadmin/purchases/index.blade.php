<x-superadmin-layout>
    ...
    <div class="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <h1 class="text-3xl font-extrabold text-slate-900 tracking-tight">Riwayat Pembelian</h1>
            <p class="text-slate-500 mt-1">Daftar transaksi kasir yang telah berhasil dilakukan.</p>
        </div>
        <a href="{{ route('superadmin.purchases.create') }}" class="inline-flex items-center gap-2 px-4 py-2.5 bg-brand-blue text-brand-yellow font-bold rounded-xl shadow-lg shadow-brand-blue/20 hover:bg-brand-blue/90 transition-all focus:ring-2 focus:ring-brand-blue focus:outline-none">
    ...
    <div class="bg-white rounded-3xl shadow-sm border border-slate-100 p-6 overflow-hidden">
        <div class="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4 border-b border-slate-100 pb-4">
            ...
            <form action="{{ route('superadmin.purchases.index') }}" method="GET" class="w-full sm:w-64 relative">
                <input type="text" name="search" value="{{ $search }}" placeholder="Cari nama pemasok..." class="w-full pl-10 pr-4 py-2 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white focus:ring-2 focus:ring-brand-blue/20 focus:border-brand-blue transition-all text-sm font-medium">
            ...
            <table class="w-full text-left text-sm text-slate-600">
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50 transition-colors">
                        <td class="px-4 py-3 font-bold text-slate-700">#TRX-{{ str_pad($purchase->id, 5, '0', STR_PAD_LEFT) }}</td>
                        <td class="px-4 py-3 text-slate-500 font-medium">{{ \Carbon\Carbon::parse($purchase->purchase_date)->translatedFormat('d F Y') }}</td>
                        <td class="px-4 py-3 font-bold text-slate-800">{{ $purchase->supplier->name }}</td>
                        <td class="px-4 py-3 text-slate-500">{{ $purchase->user->name ?? '-' }}</td>
                        <td class="px-4 py-3 text-right font-black text-brand-blue">Rp {{ number_format($purchase->grand_total, 0, ',', '.') }}</td>
                        <td class="px-4 py-3 text-center">
                            <div class="flex items-center justify-center gap-2">
                                <!-- Print Invoice Button -->
                                <button type="button" @click="invoice = {{ json_encode($purchase) }}; invoiceModalOpen = true" class="p-2 text-slate-400 hover:text-brand-blue hover:bg-blue-50 rounded-lg transition-colors" title="Cetak Struk">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z" /></svg>
                                </button>
                                
                                <!-- Delete Button (with confirmation) -->
                                <form action="{{ route('superadmin.purchases.destroy', $purchase->id) }}" method="POST" class="inline-block confirm-form" data-action="menghapus transaksi {{ $purchase->supplier->name }}">
                                    @csrf
                                    @method('DELETE')
                    ...
                    <tr>
                        <td colspan="6" class="px-4 py-16 text-center">
                            <div class="flex flex-col items-center justify-center">
                                ...
                                <p class="text-slate-500 mt-1 max-w-sm">Klik tombol "Tambah Transaksi Baru" di atas untuk mulai membeli obat ke pelanggan.</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
                    ...
                    <div id="printable-invoice" class="bg-white px-6 pt-6 pb-6" x-show="invoice">
                        ...
                        <div class="flex justify-between text-xs text-slate-600 font-medium mb-4">
                            ...
                            <div class="text-right">
                                <p>Tgl : <span x-text="new Date(invoice?.purchase_date).toLocaleDateString('id-ID')"></span></p>
                                <p>Plg : <span x-text="invoice?.supplier->name"></span></p>
                            </div>
                        </div>
                                    ...
                                    <template x-for="item in invoice?.details" :key="item.id">
                                        <tr>
                                            <td class="py-2 pr-2">
                                                ...
                                                <div class="text-[10px] text-slate-500"><span x-text="new Intl.NumberFormat('id-ID').format(item.price)"></span> / <span x-text="item.medicine?.unit?.name"></span></div>
                                            </td>
